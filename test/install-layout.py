...
def main(source):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for name, extra, expected in cases:
            ...
            if not expect(f"{name}: the example installs", not error,
                          error.strip().splitlines()[-1] if error else ""):
                continue

            seen = where(prefix)
            expect(f"{name}: the config lands in {expected}", seen == expected, seen)
            ...
            if version:
                ...
                expect(f"{name}: the version file {'ignores' if wants else 'checks'} the pointer width",
                       claims == wants)
        ...
        if expect("find_package finds the installed package", out.returncode == 0,
                  out.stderr.strip().splitlines()[-1] if out.returncode else ""):
            out = run("cmake", "--build", str(build))
            expect("a consumer builds against it", out.returncode == 0)

    return report("Where the package installs")
